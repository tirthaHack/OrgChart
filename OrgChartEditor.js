	...
	var fileInput = document.getElementById("csv"),
		...
		ceoNode.setFullName("Senthil");
		ceoNode.setImageLocation("icon.png");
		ceoNode.setComment("The CEO of this great company");
		ceoNode.resize();
		diagram.addItem(ceoNode);	
		parents.set("0",ceoNode);
			...
			reader.onload = function () {
				...
				const myArr = lines[line].toString().split(",");
				console.log(myArr);
				
		var node = new OrgChartNode(diagram, ceoNode);
		node.setBounds(ceoNode.getBounds());
		node.setImageLocation("icon.png"); 
        node.setFullName(myArr[0]);		
		diagram.addItem(node);
		node.setComment("(comment)");
		node.resize(); 
		var array = myArr[2];
		
		var linktemp = diagram.getFactory().createDiagramLink(parents.get(array), node);
		styleLink(linktemp);
		...
		ctoNode.setImageLocation("icon.png");
		ctoNode.setComment("A great person!");
		ctoNode.resize();
		diagram.addItem(ctoNode);

		var hrNode = new OrgChartNode(diagram, ceoNode);
		hrNode.setBounds(new Rect(25, 55, 60, 25));
		hrNode.setTitle("HR");
		hrNode.setFullName("Mary Johnson");
		hrNode.setImageLocation("icon.png");
		hrNode.setComment("Human Relations Manager");
		hrNode.resize();
		diagram.addItem(hrNode);

		var prNode = new OrgChartNode(diagram, ceoNode);
		prNode.setBounds(new Rect(25, 55, 60, 25));
		prNode.setTitle("PR");
		prNode.setFullName("Diana Brandson");
		prNode.setImageLocation("icon.png");
		prNode.resize();
		diagram.addItem(prNode);
		
		var prNode2 = new OrgChartNode(diagram, ceoNode);
		prNode2.setBounds(new Rect(25, 55, 60, 25));
		prNode2.setTitle("PR");
		prNode2.setFullName("Brandson ");
		prNode2.setImageLocation("icon.png");
		prNode2.resize();
		diagram.addItem(prNode2);
		...
		styleLink(link4);
		var link5 = diagram.getFactory().createDiagramLink(ceoNode, prNode2);
		styleLink(link5);*/
